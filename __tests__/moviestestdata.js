export const testData = {
    movieSummaryList: [
    {
    title: "Batman Begins",
    poster: "https://m.media-amazon.com/images/M/MV5BZmUwNGU2ZmItMmRiNC00MjhlLTg5YWUtODMyNzkxODYzMmZlXkEyXkFqcGdeQXVyNTIzOTk5ODM@._V1_SX300.jpg",
    year: "2005",
    plot: "After training with his mentor, Batman begins his fight to free crime-ridden Gotham City from corruption.",
    imdbRating: "8.2",
    imdbVotes: "1,200,605",
    language: "English, Urdu, Mandarin",
    summary_short: "N/A"
    },
    {
    title: "Batman v Superman: Dawn of Justice",
    poster: "https://m.media-amazon.com/images/M/MV5BYThjYzcyYzItNTVjNy00NDk0LTgwMWQtYjMwNmNlNWJhMzMyXkEyXkFqcGdeQXVyMTQxNzMzNDI@._V1_SX300.jpg",
    year: "2016",
    plot: "Fearing that the actions of Superman are left unchecked, Batman takes on the Man of Steel, while the world wrestles with what kind of a hero it really needs.",
    imdbRating: "6.5",
    imdbVotes: "580,423",
    language: "English",
    summary_short: "The film, pitting Ben Affleck against Henry Cavill, largely serves as an extended trailer for a slate of coming DC Comics movies like “Wonder Woman.”"
    },
    {
    title: "Batman",
    poster: "https://m.media-amazon.com/images/M/MV5BMTYwNjAyODIyMF5BMl5BanBnXkFtZTYwNDMwMDk2._V1_SX300.jpg",
    year: "1989",
    plot: "The Dark Knight of Gotham City begins his war on crime with his first major enemy being the clownishly homicidal Joker.",
    imdbRating: "7.6",
    imdbVotes: "314,665",
    language: "English, French, Spanish",
    summary_short: "Millionaire caped crusader vs. the Joker. Fast, brash and overbaked. Evil Jack's the juice."
    },
    {
    title: "Batman Returns",
    poster: "https://m.media-amazon.com/images/M/MV5BOGZmYzVkMmItM2NiOS00MDI3LWI4ZWQtMTg0YWZkODRkMmViXkEyXkFqcGdeQXVyODY0NzcxNw@@._V1_SX300.jpg",
    year: "1992",
    plot: "Batman returns to the big screen when a deformed man calling himself the Penguin wreaks havoc across Gotham with the help of a cruel businessman.",
    imdbRating: "7.0",
    imdbVotes: "254,275",
    language: "English",
    summary_short: "Penguin and Catwoman show up for the sequel. Sleeker, brighter eyeful, but with weak, drawn-out finale."
    },
    {
    title: "Batman Forever",
    poster: "https://m.media-amazon.com/images/M/MV5BNDdjYmFiYWEtYzBhZS00YTZkLWFlODgtY2I5MDE0NzZmMDljXkEyXkFqcGdeQXVyMTMxODk2OTU@._V1_SX300.jpg",
    year: "1995",
    plot: "Batman must battle former district attorney Harvey Dent, who is now Two-Face and Edward Nygma, The Riddler with help from an amorous psychologist and a young circus acrobat who becomes his sidekick, Robin.",
    imdbRating: "5.4",
    imdbVotes: "221,787",
    language: "English",
    summary_short: "Riddler and Two-Face in Gotham. Mainly gimmicky props and kitsch."
    },
    {
    title: "Batman",
    poster: "https://m.media-amazon.com/images/M/MV5BMTYwNjAyODIyMF5BMl5BanBnXkFtZTYwNDMwMDk2._V1_SX300.jpg",
    year: "1989",
    plot: "The Dark Knight of Gotham City begins his war on crime with his first major enemy being the clownishly homicidal Joker.",
    imdbRating: "7.6",
    imdbVotes: "314,665",
    language: "English, French, Spanish",
    summary_short: "The daring duo and Batgirl take on Mr. Freeze and Poison Ivy. Squashed characters and plot."
    },
    {
    title: "The Lego Batman Movie",
    poster: "https://m.media-amazon.com/images/M/MV5BMTcyNTEyOTY0M15BMl5BanBnXkFtZTgwOTAyNzU3MDI@._V1_SX300.jpg",
    year: "2017",
    plot: "A cooler-than-ever Bruce Wayne must deal with the usual suspects as they plan to rule Gotham City, while discovering that he has accidentally adopted a teenage orphan who wishes to become his sidekick.",
    imdbRating: "7.3",
    imdbVotes: "117,869",
    language: "English",
    summary_short: "Warner Bros. offers amusement at its own expense and Will Arnett gives great voice to Gotham’s brooder, but this new movie is still a commercial."
    },
    {
    title: "Batman: The Animated Series",
    poster: "https://m.media-amazon.com/images/M/MV5BOTM3MTRkZjQtYjBkMy00YWE1LTkxOTQtNDQyNGY0YjYzNzAzXkEyXkFqcGdeQXVyOTgwMzk1MTA@._V1_SX300.jpg",
    year: "1992–1995",
    plot: "The Dark Knight battles crime in Gotham City with occasional help from Robin and Batgirl.",
    imdbRating: "9.0",
    imdbVotes: "76,282",
    language: "English",
    summary_short: "N/A"
    },
    {
    title: "Batman: Under the Red Hood",
    poster: "https://m.media-amazon.com/images/M/MV5BYTdlODI0YTYtNjk5ZS00YzZjLTllZjktYmYzNWM4NmI5MmMxXkEyXkFqcGdeQXVyNTA4NzY1MzY@._V1_SX300.jpg",
    year: "2010",
    plot: "There's a mystery afoot in Gotham City, and Batman must go toe-to-toe with a mysterious vigilante, who goes by the name of Red Hood. Subsequently, old wounds reopen and old, once buried memories come into the light.",
    imdbRating: "8.1",
    imdbVotes: "49,236",
    language: "English",
    summary_short: "N/A"
    },
    {
    title: "Batman: The Dark Knight Returns, Part 1",
    poster: "https://m.media-amazon.com/images/M/MV5BMzIxMDkxNDM2M15BMl5BanBnXkFtZTcwMDA5ODY1OQ@@._V1_SX300.jpg",
    year: "2012",
    plot: "Batman has not been seen for ten years. A new breed of criminal ravages Gotham City, forcing 55-year-old Bruce Wayne back into the cape and cowl. But, does he still have what it takes to fight crime in a new era?",
    imdbRating: "8.0",
    imdbVotes: "47,785",
    language: "English",
    summary_short: "N/A"
    }
    ],
    pageNumber: 37,
    error: null
};